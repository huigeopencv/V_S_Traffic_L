...
class TrafficLightGridEnv(Env):
    ...
    * switch_time: minimum time a light must be constant before it switches (in seconds).
    ...
    * discrete: determines whether the action space is meant to be discrete or continuous
    ...
    States
        ...
        of the vehicle.
        观察值是每辆车到交叉路口的距离、唯一标识车辆所在边缘的数字以及车辆的速度。
    ...
    Actions
        ...
        动作空间由一系列浮动变量组成，范围为0-1，指定交通灯是否应该切换。
        这些动作从左到右，然后从上到下发送到网格中的红绿灯。

    Rewards
        The reward is the negative per vehicle delay minus a penalty for
        switching traffic lights
        奖励是每辆车的负延误减去切换红绿灯的惩罚
    ...
    Additional
        ...
        vehicles.
        一旦车辆到达网络的末端，就会重新路由至其原始路线的起点，以确保车辆数量不变。
    ...
    grid_array : dict
        ...
        包含交通灯网格信息的数组，如道路长度、row_num、col_num、初始车辆数量
    rows : int
    ...
    direction : np array [num_traffic_lights]x1 np array
        ...
        多维阵列跟踪交通信号灯的流向。0表示从上到下的流量，1表示从左到右的流量
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        if self.discrete:
            ...
            # should happen
            # 将小于0的值转换为0，大于0的值则转换为1。0表示不应切换方向，1表示应进行切换
    ...
    def _convert_edge(self, edges):
        ...
        从左下垂直边开始，向右再向上，因此左下垂直边缘为零，旁边的右边缘为1。
        ...
        before bottom.
        数字沿着最下面的列分配，然后是最下面的行，然后是第二个最低的列，等等。左在右之前，上在下。
    ...
    def get_closest_to_intersection(self, edges, num_closest, padding=False):
        ...
        对于边中的每个边，返回最靠近交叉点（它们正朝着的交叉点）的边中最接近的车辆的id（veh_id）。
        ...
        此功能不检查边缘是否朝向交叉路口，只获取最接近其边缘末端的车辆。
        ...
        如果边缘上的车辆少于num_closest，则如果padding参数设置为True，则函数通过添加空字符串“”而不是车辆ID来执行填充。
        ...
        例如，考虑以下网络，该网络由ID为“edge0”、“edge1”、“ledge2”和“edge3”的4条边组成，
        这些边的编号都是朝向交叉路口x行驶的车辆。编号为n的车辆的ID为“veh{n}”（边“veh0”，“veh1”…）。
...
class TrafficLightGridPOEnv(TrafficLightGridEnv):
    ...
    * num_observed: number of vehicles nearest each intersection that is
    ...
    States
        ...
        观察是指在每个交叉路口中最靠近交通灯的观察车辆数量、唯一标识车辆所在边缘的数字以及车辆速度。
    ...
    def get_state(self):
        ...
        返回self.num_observed最接近每个交通灯的车辆数量，以及每辆车辆的速度、到十字路口的距离、边缘号交通灯状态。这是部分观察到的
        """
        speeds = []
        dist_to_intersec = []
        edge_number = []
...
class TrafficLightGridBenchmarkEnv(TrafficLightGridPOEnv):
    """Class used for the benchmarks in `Benchmarks for reinforcement learning in mixed-autonomy traffic`.
    用于“混合交通中强化学习的基准”中的基准的类
    """
...
class TrafficLightGridTestEnv(TrafficLightGridEnv):
    ...
    此类重写了交通灯网格的RL方法，因此我们可以测试构造而无需指定RL方法
    """
