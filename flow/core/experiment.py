...
class Experiment:
    ...
    此类充当网络和环境的运行者。为了在没有指定网络中RL智能体动作的方法的情况下使用它来运行网络和环境，请键入以下内容：
    ...
    最后，如果您想绘制和可视化结果，这个类可以从sumo生成的排放文件生成csv文件。这些文件将包含网络中每辆车在每个时间步的速度、位置、边缘等。
    ...
    custom_callables : dict < str, lambda >
        ...
        keyed by the str.
        字符串和lambda函数对应于我们想要从环境中提取的一些信息。lambda将在每个步骤被调用以从env中提取信息，并将其存储在str键控的dict中。
    env : flow.envs.Env
