...
class KernelVehicle(object, metaclass=ABCMeta):
    ...
    methods for:
    该内核子类用于与所有车辆相关组件的模拟器交互。具体来说，此类包含以下方法：
    ...
    * Interacting with the simulator: This includes apply acceleration, lane
      ...
      methods.
      与模拟器交互：这包括向模拟器中的特定车辆应用加速、车道变更和路线命令。
      此外，存在从网络中添加或移除特定车辆的方法，并在每个模拟步骤之后更新内部状态信息，以支持并可能加快所有状态获取方法。
    * Visually distinguishing vehicles by type: In the case when some vehicles
      ...
      按类型目视区分车辆：在某些车辆由强化学习代理或其他控制器控制的情况下，这些方法可用于在RL/驱动、人类观察和人类未观察的渲染过程中目视区分车辆。
      例如，traci模拟器将RL车辆渲染为红色，观察到的人类车辆渲染为青色，未观察到的人车辆渲染为白色。在没有RL/致动剂的情况下，所有车辆均为白色。
    * State acquisition: Finally, this methods contains several methods for
      ...
      be done by calling:
      状态获取：最后，该方法包含从特定车辆获取状态信息的几种方法。例如，如果您想从环境中获取车辆的速度，可以通过调用：
