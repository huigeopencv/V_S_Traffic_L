...
class TraCIVehicle(KernelVehicle):
    ...
    def __init__(self,
        ...
        """
        2023.11.26做受VSL控制车辆IDs的修改
        此处作注释
        self.__vsl_ids = []
        """
        self.__vsl_ids = []  # ids of vsl vehicles
        self.__observed_ids = []  # ids of the observed vehicles

        # vehicles: Key = Vehicle ID, Value = Dictionary describing the vehicle
        # Ordered dictionary used to keep neural net inputs in order
        # 车辆：关键字 = 车辆ID，值 = 描述车辆的字典有序字典用于保持神经网络输入有序
        ...
        # 创建一个sumo_observations变量，该变量将携带给定时间段内车辆状态的所有信息
        self.__sumo_obs = {}

        # total number of vehicles in the network 网络中的车辆总数
        self.num_vehicles = 0
        # number of rl vehicles in the network 网络中rl车辆的数量
        self.num_rl_vehicles = 0
        # number of vehicles  loaded but not departed vehicles 装载但未离开的车辆数量
    ...
    def remove(self, veh_id):
        ...
        if veh_id in self.__human_ids:
            ...
            if veh_id in self.__vsl_ids:
                self.__vsl_ids.remove(veh_id)
            """
            2023.11.26做受VSL控制车辆IDs的修改
            此处作注释
            if veh_type == 'vsl':
    ...
    2023.11.26做受VSL控制车辆IDs的修改
    此处作注释
    def get_vsl_ids(self):
        return self.__vsl_ids
    """
    def get_vsl_ids(self):
        """See parent class."""
        return self.__vsl_ids
    def get_road_id(self,veh_id):
    ...
    def set_speed(self, veh_id, max_speed):
        """See parent class."""
        self.kernel_api.vehicle.setSpeed(veh_id, max_speed)
